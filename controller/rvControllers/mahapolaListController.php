...
<?php
        ...
        elseif(isset($_POST['display-report'])){
            ...
            $semester = mahapolaModel::getSemDigit($year_month, $connect);
            ...
            if(mysqli_num_rows($students) > 0){
                ...
                while($stu = mysqli_fetch_assoc($students)){
                    ...
                    $check_has_reconcilation_record = mahapolaModel::checkHasRecord($degree,$batch_no,$year,$month,$connect);

                    if(mysqli_num_rows($check_has_reconcilation_record) == 1){
                        $update_reconcilation = mahapolaModel::updateRecocilation($degree,$batch_no,$year,$month,$eligible_count_m,$eligible_count_o,$non_eligible_count,$connect);
                    }
                    else{
                        $insert_reconcilation = mahapolaModel::insertReconcilation($degree,$batch_no,$year,$month,$eligible_count_m,$eligible_count_o,$non_eligible_count,$connect);
                    }

                $degree_name = mahapolaModel::getDegreeName($degree,$connect);
                ...
                elseif($report_type == 'monthlyReconciliationReport'){
                    ...
                    $cur_reco_result = mahapolaModel::getCurrentMonthResult($degree,$batch_no,$year,$month,$connect);

                    if($month == 1){
                        $prev_month = 12;
                        $prev_year = $year - 1;

                        $prev_reco_result = mahapolaModel::getPreviousMonthResult($degree,$batch_no,$prev_year,$prev_month,$connect);

                    }
                    else{
                        $prev_month = $month - 1;

                        $prev_reco_result = mahapolaModel::getPreviousMonthResult($degree,$batch_no,$year,$year,$prev_month,$connect);
                    }
                    ...
                    if(mysqli_num_rows($prev_reco_result) == 1){
                        ...
                        header('Location:../../view/reportViewer/rvReconcilationReportV.php');
            ...
            class getPdf extends FPDF {
               ...
               function displayDetail($eli_list,$connect){
                    for($i =0; $i<count($eli_list) ; $i++){
                        ...
                        $student_det = mahapolaModel::getStudentDetails($eli_list[$i],$connect);
                        $stu_det = mysqli_fetch_assoc($student_det);
            ...
            class getPdf extends FPDF {
               ...
               function displayDetail($ineli_list,$connect){
                    for($i =0; $i<count($ineli_list) ; $i++){
                        ...
                        $student_det = mahapolaModel::getStudentDetails($ineli_list[$i],$connect);
                        $stu_det = mysqli_fetch_assoc($student_det);
        ...
        elseif(isset($_POST['recoPDF'])){
            ...
            class getPdf extends FPDF {
                ...
                function displayInfo($connect){
                    ...
                    $cur_reco_result = mahapolaModel::getCurrentMonthResult($degree_id,$batch_no,$year,$month,$connect);
                    $cur_result = mysqli_fetch_assoc($cur_reco_result);
